...
pub fn download(
    ...
    // Оборванное соединение выглядит как обычный конец потока: сервер сказал
    // «столько-то байт», а пришло меньше — файл молча получился обрезанным.
    // Из-за этого встреча на девятнадцать минут расшифровывалась как шесть.
    if total > 0 && done < total {
        drop(file);
        let _ = std::fs::remove_file(target);
        return Err(anyhow!(
            "загрузка оборвалась: {:.1} из {:.1} МБ",
            done as f64 / 1e6,
            total as f64 / 1e6
        ));
    }
